...
path_grf = os.path.join(
    path_opensim_core,
    'results_pool/averages/sub03_strirt_cond00000_speed03000_grf_move_averages.xml')
path_markers = os.path.join(
    path_opensim_core,
    'sub03/ses20250306/sub03_strirt_cond00000_speed03000_0001_0003_trc.trc')
path_kinematics = os.path.join(
    path_opensim_core,
    'results_pool/averages/sub03_strirt_cond00000_speed03000_ik_move_averages.sto')
prescr_states = os.path.join(
    path_opensim_core,
    'results_pool/averages/sub03_strirt_cond00000_speed03000_rad_averages.sto')
...
guess_file          = os.path.join(path_opensim_core, 'results_pool/averages/guesses/sub03_strirt_cond00000_speed03000_mi_emg_solution.sto') 
...
path_solution       = os.path.join(path_opensim_core, 'results_pool/averages/guesses/sub03_strirt_cond00000_speed03000_20res_02mesh.sto')
path_log            = os.path.join(path_opensim_core, 'results_pool/averages/guesses/sub03_strirt_cond00000_speed03000_20res_02mesh.log')

# path_solution       = 'example_muscle_driven_26_states.sto'
# path_log            = 'example_muscle_driven_26_states.log'  
...
def muscleDrivenStateTracking(
    ...
    """   Changes tested in this iteration (1):
    - *Switching out prescribed trial to rearfoot strikes
    - try implementing a get forces function to retrieve energy return from generalizedspringforce
    - ^ also implementing a get spring forces function for mtp joint
    - *Shifted weighting to place higher emphasis on hips, knees, ankles, 
    and also apply global weighting to ALL coordinates using customization states tracking weight
    - added in validation check for computing negative muscle forces, muscle mechanical parameters
    
    On iteration (2) now;
    - started tracking pelvis_ty coordinate for a guess file
    - changed effort weight on pelvis coordinates to 100 from 10
    

    On iteration (3)
    - adjusted upper arm torque actuators to weaker strength
    - changed the convergence tolerance to 1e-2
    - switched weight set to original problem's weight set, particularly for state tracking, effort weight both being decreased
    - **turning off any tracking at all for pelvis_ty coordinate
    
    **_test_2 is the reset weights run, test_3 is reset & pelvis track, test_4 reduces convergence tolerance to e-2

    TO-DO for iteration (5):
    -** Finally fix CoP table function.
    - adjust conversion functions to include first & second derivatives for speed and acceleration
    - add in goal for tibial acceleration, speed, orientation tracking
    - switch weights back to heavier states tracking
    - Write in joint contact force function to retrieve from solution
    *Trying to replicate _test_49.sto results by defaulting global states tracking weight
    
    Iterations 11-onward:
    previous final problem of any reserves <10 won't converge; therefore, trying to restart with new set of kinematics to see if 
    arm flexors were the issue, and also prescribe new set of subt data locked at 0.
    ^adding in gloabal states tracking weight this time
    If this also doesn't work, consider adjusting upper body coord weights

    **It actually ended up being the guess that was the issue; by swapping in a model with diffferent contact sphere height, different subtalar
    kinematics in the mi_emg solution, this seemed to cause wonky floating of pelvis_ty coordinate
    The correct workflow now: iksolutions generated w/ LOCKED subtalars --> inverseEMG problems solved with comliant tendons added in & subtalars unlocked
    ^^use the above inverseEMGs as guesses in higher reserve guess generation iterations.  
    
    **Strongly consider calibrating muscle parameters, adding in metabolic probe for desired muscles
    *The problem/sequence as is works right now when starting from mocoinverse solutions on locked out subtalars as an initial guess
    - Now I'm trying to see if the solution's accuracy can be improved at all by adding in tibial acceleration, orientation, velocity goals.
    ^to do this, you'd have to write in the accelerations into a timeseriestable vec3 for setAccelerationReference()
    **also adjust the bounds settings
    
    **Now that I'm using a model with updated muscle parameters, I'm trying to re-run the pipeline by applying forces to different bodies;
    -_test_40 is running without any periodicity constraints at 0.05 mesh 20 reserves, force applied to toes
    -_test_41 with periodicity goals for coords and muscles and is 0.05 mesh with 20 reserves applying to toes
    -_test_42 is 0.1 mesh with 200 reserves applying to toes
    -_test_43 is 0.2 mesh with 200 reserves applying to toes
    -_test_44 still with periodicity at is 0.2 mesh with 200 reserves applying to calc bodies
    
    46_old_model has no perdiodicity and speeds included in reference file
    SIDE NOTE: avoiding egregious pelvis_ty errors was from some combination of including speed coord values, placing weight on pelvis actuator effort to 100, switching weights sets, removing/adding pelvis_ty tracking
    For adding periodicity, 
    I'm trying to see if using non-periodic solution as a guess will help (45)
    ^Also, if using non-compliant tendons in all muscles will help (45_new_model_periodic)

    Generating results:
    *Problem instead converges faster, with more accuracy especially on 3 m/s speeds, when mi solutions used as guesses immediately to final solution
    - Prior results are saved within /dump/ folder
    - This iteration of results features tighter contraint tolerance, 0 lumbar coordinate tracking weight
    """
    ...
    compute_sto_stdevs(stoFileName=converted_states, outputCsvFileName=state_stdevs)
    ...
    coordinates_table = osim.TimeSeriesTable(converted_states)
    ...
    coord_bounds = {}
    reference_data = {}
    state_bounds = set_state_bounds(coord_bounds=None, reference_states_path=converted_states, kinematicLimits=None)
    ...
    exp_states = osim.TableProcessor(converted_states).process()
    ...
    # Try adding in a tibial orientation tracking goal that also tracks tibial accelerations to see if it improves ankle angle outputs
    # ================================================================================================================================
    exp_state_labels = exp_states.getColumnLabels()
    shank_table = osim.TimeSeriesTable(exp_states)
    state_vars = model.getStateVariableNames()
    activation_vec = osim.Vector(shank_table.getNumRows(), 0.02)
    for isv in np.arange(state_vars.getSize()):
        state_var = state_vars.get(int(isv))
        if not state_var in exp_state_labels:
            shank_table.appendColumn(state_var, activation_vec)


    # tibia_orientaion_goal = osim.MocoOrientationTrackingGoal('tibia_orientation_goal', 
    #                                                             weights['tibia_orientation_weight'])
    # tibia_orientaion_goal.setStatesReference(osim.TableProcessor(shank_table))
    # paths = osim.StdVectorString()
    # paths.push_back('/bodyset/tibia_r')
    # paths.push_back('/bodyset/tibia_l')
    # tibia_orientaion_goal.setFramePaths(paths)
    # tibia_orientaion_goal.setEnabled(True)
    
    # problem.addGoal(tibia_orientaion_goal)

    # tibia_speed_goal = osim.MocoAngularVelocityTrackingGoal('tibia_angular_velocity_goal', 
    #                                                         0.1 * weights['tibia_orientation_weight'])
    # tibia_speed_goal.setStatesReference(osim.TableProcessor(shank_table))
    # paths = osim.StdVectorString()
    # paths.push_back('/bodyset/tibia_r')
    # paths.push_back('/bodyset/tibia_l')
    # tibia_speed_goal.setFramePaths(paths)
    # tibia_speed_goal.setEnabled(True)

    # problem.addGoal(tibia_speed_goal)

    # tibia_acceleration_goal = osim.MocoAccelerationTrackingGoal('tibial_acceleration_goal', 
    #                                                         0.1 * weights['tibia_orientation_weight'])
    # tibia_acceleration_goal.setStatesReference(osim.TableProcessor(shank_table))
    # paths = osim.StdVectorString()
    # paths.push_back('/bodyset/tibia_r')
    # paths.push_back('/bodyset/tibia_l')
    # tibia_acceleration_goal.setFramePaths(paths)
    # tibia_acceleration_goal.setEnabled(True)

    # problem.addGoal(tibia_acceleration_goal)
    ...
    # Grab the model output that was used too
    end_model_path = 'sub03_muscle_adjusted_model.osim'
    ...
    log_file = 'example_muscle_driven_output.log'
    ...
    # end_model_path = 'example_muscle_adjusted_model.osim'
    # model.printToXML(end_model_path)
    ...
    path_comp = path_solution.replace('.sto', '_errors.sto')
    ...
    moments_file = path_solution.replace('.sto', '_leg_net_moments.sto')
...
# insert guess generation sequencing
# ** add in coordinate based spring generalized force around mtp 
# build out validation pipeline:
"""

includes:
- **retrieve grfs, joint moments from solutions
- rms errors between experimental and marker trajectories from ik solutions (ideally <0.7 cm)
- **rms errors between experimental coordinate data and simulative solution coord trajectories
- **mean rms errors between experimental grfs and simulation grfs
- *visual matchings of EMG with muscle activations - RMS for normalized emg data that is close to 0.15
"""
# then build results analysis pipeline/retrieve_results script:
"""this would be based upon:
- normalized tendon force
- negative muscle forces
- provided force from passive coorindate-based force element
- tendon strain
"""
# ?Adjusting pelvis tracking to reduce abnormal leg movement?
# check paper for reasoning of tracking torso
# ** next, add in spped translations and pelvis_tx tracking goal to problem
