...
from osimFunctions import calculate_plate_torques_from_cop_forces
    ...
    for subject_id in subjects:
        ...
        for i, strike_type in enumerate(strike_types):
            for j, speed_str in enumerate(speeds):
                ...
                path_grf = os.path.join(avg_inputs, f"{base_filename}_grf_move_averages.xml") 
                path_grf_data = os.path.join(avg_inputs, f"{base_filename}_grf_move_averages.sto") 
                ...
                try:
                    if not args.dry_run:
                        ...
                        calculate_plate_torques_from_cop_forces(grf_file_path=path_grf_data, output_file_path=path_grf_data)
