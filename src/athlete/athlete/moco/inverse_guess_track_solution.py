...
from tracking_problem_construct import muscleDrivenStateTracking
...
if __name__ == "__main__":
    ...
    base_data_directory = "/home/lisca/biomec/data/"
    avg_inputs = os.path.join(base_data_directory, '20250306_dataset/results_pool/averages/')
    models_dir = os.path.join(base_data_directory, '20250306_dataset/')
    ...
    # Run first iteration of tracking problem using Moco Inverse Kinematics as initial guess
    ...
    for subject_id in subjects:
            ...
            if f"sub{subject_id}" in item and os.path.isdir(os.path.join(models_dir, item)):
                ...
                for ses_item in os.listdir(subject_dir):
                    if 'ses' in ses_item and os.path.isdir(os.path.join(subject_dir, ses_item)):
                        ...
                        print(f"  Searching for subject model in {session_dir}")
                        break
                break
        
        if not subject_dir or not session_dir:
            print(f"  ERROR Could not find subject directory for sub{subject_id}")
        ...
        if not path_model:
            print(f"  ERROR Model file not found for sub{subject_id}")
            failed_processing.append(f"Moco: sub{subject_id} - Model file not found")
            continue
        
        print(f"  Found: Model: {os.path.basename(path_model)}")
        if path_musclepaths:
            print(f"  Found Muscle paths: {os.path.basename(path_musclepaths)}")
        else:
            print(f"  Warning:  Muscle paths file not found (proceeding without)")
            ...
            for j, speed_str in enumerate(speeds):
                ...
                if not os.path.exists(path_states):
                    ...
                    try:
                        ...
                        print(f"    ERROR Error generating states file: {e}")
                ...
                try:
                    ...
                    print(f"    ERROR Error during Moco optimization: {e}")
